...
vi.mock('./api', () => ({ sendAccounting: vi.fn(), sendAccountingDcc3: vi.fn(), checkDocumentNos: vi.fn() }))
vi.mock('../../shared/toast', () => ({ toast: { ok: vi.fn(), info: vi.fn(), err: vi.fn() } }))
import { sendAccounting, checkDocumentNos, type BoardCard } from './api'
...
describe('BatchSendAccountingModal — enter many numbers at once', () => {
  ...
  it('sends only the filled rows and skips the blanks, then closes', async () => {
    vi.mocked(checkDocumentNos).mockResolvedValue({ existing: [] })
  ...
  it('flags the same number typed on two rows live and disables send (sends nothing)', () => {
    render(<BatchSendAccountingModal cards={cards} onClose={vi.fn()} onDone={vi.fn().mockResolvedValue(undefined)} />)
    const inputs = screen.getAllByPlaceholderText('Contract No')
    fireEvent.change(inputs[0]!, { target: { value: 'SAME' } })
    fireEvent.change(inputs[1]!, { target: { value: 'SAME' } })

    // Both colliding rows are flagged before any submit.
    expect(screen.getAllByRole('alert').length).toBe(2)
    expect(screen.getByRole('button', { name: /Gửi 2 hồ sơ/ })).toBeDisabled()
    expect(sendAccounting).not.toHaveBeenCalled()
  })

  it('blocks the whole batch when a number is already taken (pre-flight), sending nothing', async () => {
    vi.mocked(checkDocumentNos).mockResolvedValue({ existing: ['26-CC-1'] })
    const onClose = vi.fn()
    render(<BatchSendAccountingModal cards={cards} onClose={onClose} onDone={vi.fn().mockResolvedValue(undefined)} />)

    fireEvent.change(screen.getAllByPlaceholderText('Contract No')[0]!, { target: { value: '26-CC-1' } })
    fireEvent.change(screen.getAllByPlaceholderText('Contract No')[2]!, { target: { value: '26-CC-3' } })
    fireEvent.click(screen.getByRole('button', { name: /Gửi 2 hồ sơ/ }))

    await waitFor(() => expect(screen.getByRole('alert')).toBeInTheDocument())
    expect(sendAccounting).not.toHaveBeenCalled() // nothing half-sent
    expect(onClose).not.toHaveBeenCalled()
  })
})
